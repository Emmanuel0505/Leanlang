...
MAX_REVISIONS = 1  # cuantas veces el critico puede devolver al selector


# ---- Triaje de entrada -------------------------------------------------------
def supervisor_node(state: BlueprintState) -> dict:
    """Nodo de entrada: registra el inicio. La decision de ruta la toma route_entry."""
    return {"messages": [trace("supervisor", "Triaje: orquestando el diseno de validacion.")]}
...
def route_after_critic(state: BlueprintState) -> str:
    ...
    if not review.get("passed", True) and revisions < MAX_REVISIONS:
...
def bump_revision_node(state: BlueprintState) -> dict:
    ...
    return {
        ...
        "messages": [trace("supervisor", "Re-disenando experimentos segun feedback del critico.")],
    }
...
def human_hypotheses_node(state: BlueprintState) -> dict:
    ...
    if isinstance(edited, dict) and edited.get("hypotheses"):
        return {
            ...
            "messages": [trace("human", "Hipotesis editadas por el usuario.")],
        }
    return {"messages": [trace("human", "Hipotesis aceptadas por el usuario.")]}
...
def human_prioritization_node(state: BlueprintState) -> dict:
    ...
    if isinstance(edited, dict) and edited.get("prioritization"):
        return {
            ...
            "messages": [trace("human", "Priorizacion ajustada por el usuario.")],
        }
    return {"messages": [trace("human", "Priorizacion aceptada por el usuario.")]}
...
def human_approval_node(state: BlueprintState) -> dict:
    ...
    return {"messages": [trace("human", msg)]}
