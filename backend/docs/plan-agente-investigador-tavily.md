...
- El gate del Supervisor cortocircuita si ya hay `research` o `problem` en el estado → en revisiones del Crítico, resumes de interrupts y re-entradas por `route_entry` no se vuelve a investigar.
- Se de-duplican queries antes de llamar a Tavily.
- Un cambio importante de idea implica un nuevo blueprint (nuevo `thread_id`) → la investigación se rehace de forma natural.
